...
/////// MAIN PROGRAM ///////
	...
	print "<h2>Are You Game Enough?</h2>";
	...
	else if ($action=='Black, Drop a Bomb'|| $action=='Gold, Drop a Bomb')
	{
		...
		else
		{
			...
			if (($Grid[$x][$y]==$hitblack)||($Grid[$x][$y]==$hitgold))
			{
				print '<embed hidden=\"true\" autostart=\"true\" src=\"$hitsound\"></embed>';
			}
	...
	$blackcount=countcolor($hitblack);
	
	//print "I see $blackcount black squares.";
	
	if ($blackcount==5)
	{
	print "Black's ship is sunk!";	
	}
	
	$goldcount=countcolor($hitgold);
	
	//print "I see $goldcount gold squares.";
	
	if ($goldcount==5)
	{
	print "Gold's ship is sunk!";	
	}
